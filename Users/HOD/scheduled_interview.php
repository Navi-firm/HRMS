<div class="block-content collapse in">
    <table id="shortlisted" class="table table-striped" cellspacing="0" width="100%">
        ...
        $application = mysql_query("SELECT application_id, application_title, applicant_name, DATE_FORMAT(DATE(application_date), '%D %M %Y') AS app_date, applicant_contact, applicant_email, application_status, resume, cover_letter, interview_date FROM applications WHERE application_status = 'Schedule Interview' AND department = '$my_department' ORDER BY application_date DESC ") or die(mysql_error());
        ...
        if(mysql_num_rows($application) != 0){
            ...
            While($row = mysql_fetch_assoc($application)){ ?>
                ...
                <tr>
                    ...
                    <td>
                        <a href="set_interview.php?application_id=<?php echo $row['application_id']; ?>" alt="edit" class="btn btn-info btn-sm"><i class="icon-edit-sign"></i> Review </a>
                    </td>
                </tr>
                </tbody>
            <?php } ?>
